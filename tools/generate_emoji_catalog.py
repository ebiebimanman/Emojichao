...
def main():
    if len(sys.argv) != 5:
        raise SystemExit("usage: generate_emoji_catalog.py emoji-test.txt ja.json en.json output.json")

    emoji_test, japanese_path, english_path, output_path = sys.argv[1:]
    ...
    for line in Path(emoji_test).read_text(encoding="utf-8").splitlines():
        ...
        if emoji in seen:
            continue
        seen.add(emoji)
        japanese_entry = japanese.get(emoji, {})
        english_entry = english.get(emoji, {})
